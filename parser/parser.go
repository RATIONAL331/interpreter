...
func (p *Parser) parseExpressionStatement() ast.Statement {
	...
	if p.peekTokenIs(token.SEMICOLON) { // 세미콜론은 선택적
		p.nextToken()
	}
	return statement
}
...
func (p *Parser) parseFunctionParameters() []*ast.Identifier {
	...
	for p.peekTokenIs(token.COMMA) {
		p.nextToken()
		p.nextToken()
		innerIdent := &ast.Identifier{Token: p.curToken, Value: p.curToken.Literal}
		identifiers = append(identifiers, innerIdent)
	}
...
func (p *Parser) parseMacroLiteral() ast.Expression {
	macroLiteral := &ast.MacroLiteral{Token: p.curToken}

	if !p.expectPeek(token.LPAREN) {
		return nil
	}

	macroLiteral.Parameters = p.parseFunctionParameters()

	if !p.expectPeek(token.LBRACE) {
		return nil
	}

	macroLiteral.Body = p.parseBlockStatement()
